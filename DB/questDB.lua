...
local function IsPreReqComplete(quest, group, QuestDB)
    if type(quest) ~= "table" then return true end
    group = group or addon.currentGuide and addon.currentGuide.group
    local guideQuestDB = type(addon.QuestDB) == "table" and group and
                             addon.QuestDB[group]
    QuestDB = type(QuestDB) == "table" and QuestDB or guideQuestDB or
                  addon.QuestDBLegacy or {}
...
-- Returns true when the known prerequisites are complete, false plus the
-- missing quest IDs when they are not, and nil when RXP has no record for the
-- quest. Unknown data must never cause a guide task to be skipped.
local function EvaluateStandalonePrerequisite(prerequisite, state)
    if type(prerequisite) ~= "table" or
        type(prerequisite.clauses) ~= "table" then return end

    state = type(state) == "table" and state or {}
    local rewarded = type(state.rewarded) == "table" and state.rewarded or {}
    local active = type(state.active) == "table" and state.active or {}
    local function IsRewarded(questId)
        return rewarded[questId] or addon.IsQuestTurnedIn(questId)
    end
    local function IsActive(questId)
        return active[questId] or rewarded[questId] or
                   (addon.IsOnQuest and addon.IsOnQuest(questId)) or
                   addon.IsQuestTurnedIn(questId)
    end
    ...
    for _, clause in ipairs(prerequisite.clauses) do
        ...
        for _, requirement in ipairs(clause) do
            local questId = tonumber(requirement.id)
            local requirementState = requirement.state
            local requirementSatisfied
            if questId and requirementState == "R" then
                requirementSatisfied = IsRewarded(questId)
            elseif questId and requirementState == "A" then
                requirementSatisfied = IsActive(questId)
            elseif questId and requirementState == "N" then
                requirementSatisfied = not IsActive(questId)
            end
            if not requirementSatisfied then
                satisfied = false
        ...
        for _, questId in ipairs(missing) do
            if not missingSeen[questId] then
                missingSeen[questId] = true
                table.insert(allMissing, questId)
            end
        end
    end
...
function addon.GetQuestPreReqState(id, group, state)
    id = tonumber(id)
    ...
    local quest = QuestDB and QuestDB[id]
    if type(quest) ~= "table" then return end

    local previous = quest.previousQuest
    local previousType = type(previous)
    if previousType ~= "number" and previousType ~= "table" then return true, {} end

    local missing = {}
    if previousType == "table" then
        if quest.preQuestAny then
            for _, questId in ipairs(previous) do
                if addon.IsQuestTurnedIn(questId) then return true, {} end
                table.insert(missing, questId)
            end
        else
            for _, questId in ipairs(previous) do
                if not addon.IsQuestTurnedIn(questId) then
                    table.insert(missing, questId)
                end
            end
        end
    else
        if addon.IsQuestTurnedIn(previous) then return true, {} end

        table.insert(missing, previous)
        local previousQuest = QuestDB[previous]
        local alternatives = quest.uniqueWith and previousQuest and
                                 previousQuest.uniqueWith
        if alternatives then
            if type(alternatives) ~= "table" then alternatives = {alternatives} end
            for _, questId in pairs(alternatives) do
                if addon.IsQuestTurnedIn(questId) then return true, {} end
                if questId ~= previous then table.insert(missing, questId) end
            end
        end
    end
...
function addon.IsGuideQuestActive(id)
    ...
    local group = addon.currentGuide and addon.currentGuide.group
    local QuestDB = type(addon.QuestDB) == "table" and group and
                        addon.QuestDB[group] or addon.QuestDBLegacy or {}
...
function addon.CalculateTotalXP(flags)
    ...
    local QuestDB = type(addon.QuestDB) == "table" and grp and
                        addon.QuestDB[grp] or addon.QuestDBLegacy or {}
    local totalXp = 0
    flags = flags or 0
