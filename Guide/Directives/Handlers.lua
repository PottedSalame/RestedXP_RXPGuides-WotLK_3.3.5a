...
events.dmf = "CALENDAR_UPDATE_EVENT_LIST"
events.nodmf = events.dmf
events.holiday = events.dmf
...
-- Modern UNIT_SPELLCAST_* events provide (unit, castGUID, spellID), while the
-- 3.3.5 client provides (unit, spellName, spellRank, castID).  Keep the raw
-- cast ID out of spell comparisons and fall back to the localized spell name
-- for legacy clients.
local function GetUnitSpellcastData(unit, arg2, arg3)
    if gameVersion == 30300 then
        return unit, nil, type(arg2) == "string" and arg2 or nil
    end
    local spellId = tonumber(arg3)
    return unit, spellId, spellId and GetSpellInfo(spellId) or nil
end

local function UnitSpellcastMatches(spellId, spellName, wantedId)
    wantedId = tonumber(wantedId)
    if not wantedId or wantedId <= 0 then return false end
    if spellId == wantedId then return true end
    if type(spellName) ~= "string" then return false end
    local wantedName = GetSpellInfo(wantedId)
    return type(wantedName) == "string" and spellName == wantedName
end
...
local function RequestQuestData(id)
    ...
    if nrequests < N_REQUESTS or requests[id] == 0 then
        ...
        if (not requests[id] or ctime - requests[id] > 3) then
            local ok
            ok, isLoaded = pcall(HaveQuestData, id)
            if not ok then isLoaded = false end
        end
        ...
        if isLoaded then
            ...
            local incomplete
            local firstObjective = questInfo[1]
            if (#questInfo == 1 and (type(firstObjective) ~= "table" or
                firstObjective.type == "" or not firstObjective.type)) or
                #questInfo == 0 then
                questInfo[1] = {
                ...
                for _,obj in pairs(questInfo) do
                    if type(obj) ~= "table" then
                        ...
                        if type(obj.text) ~= "string" or
                            obj.text:sub(1,1) == " " then
                            incomplete = true
                            --print(id,_)
                        end
                    end
...
function addon.GetQuestName(id)
    ...
    local questNameCache = RXPCData.questNameCache
    local persistentNames = RXPData and RXPData.questNames
    local group = addon.currentGuide and addon.currentGuide.group
    local questDB = type(addon.QuestDB) == "table" and group and
                        addon.QuestDB[group] or addon.QuestDBLegacy
    local keepData = type(questDB) == "table" and
                         type(questDB[id]) == "table"
    local name

    local function CacheQuestName(value)
        if type(value) ~= "string" or value == "" then return end
        if keepData and type(persistentNames) == "table" then
            persistentNames[id] = value
        else
            questNameCache[id] = value
        end
    end

    local function GetCachedName()
        local persistent = type(persistentNames) == "table" and
                               persistentNames[id]
        if type(persistent) == "string" and persistent ~= "" then
            return persistent
        end
        local transient = questNameCache[id]
        if type(transient) == "string" and transient ~= "" then
            return transient
        end
    end
...
function addon.SetElementComplete(self, disable, skipIfInactive)
    ...
    if not wasCompleted and element.OnComplete and active then
...
-- A turn-in element is also indexed by its localized name on 3.3.5 because the
...
local function IsQuestTurnedInLater(id)
    ...
    for _, element in pairs(addon.questTurnIn) do
        if type(element) == "table" and tonumber(element.questId) == id then
            return true
        end
    end
    return false
...
local function SkipMissingQuestPreReqs(self, id)
    local element = self and self.element
    local step = element and element.step
    if not (element and step and step.active and
        addon.settings.profile.skipMissingPreReqs) then return false end
    if not addon.GetQuestPreReqState or addon.skipPreReq[id] or
        element.multiple or IsQuestTurnedIn(id) or IsOnQuest(id) then
        return false
    end

    local complete, missing = addon.GetQuestPreReqState(id,
        addon.currentGuide and addon.currentGuide.group)
    if complete ~= false then return false end

    local tooltip = addon.colors.tooltip .. L("Missing pre-requisites") .. ":|r"
    for _, questId in ipairs(missing or {}) do
        local name = addon.GetQuestName(questId) or
                         fmt("%s %d", _G.QUESTS_LABEL or "Quest", questId)
        tooltip = fmt("%s\n%s%s (%d)", tooltip, addon.icons.turnin,
                      name, questId)
    end
    element.tooltip = tooltip
    element.icon = addon.icons.error
    element.skippedMissingPreReqs = true
...
function addon.functions.accept(self, ...)
    if type(self) == "string" then -- on parse
        ...
        if event == "QUEST_ACCEPTED" then
            questId = addon.NormalizeQuestAcceptedId and
                          addon.NormalizeQuestAcceptedId(arg1, questId) or
                          questId or arg1
        ...
        if step.active or element.retrieveText or
            ...
            if autoAccept then
                ...
                -- 3.3.5 gossip returns offered quests by title.  Register the
                -- title authored in the guide immediately instead of waiting
                -- for an unreliable pre-accept server name query.
                if addon.CacheGuideAcceptTitle then
                    addon.CacheGuideAcceptTitle(element)
                end
            end
        ...
        local skippedMissingPreReq = SkipMissingQuestPreReqs(self, id)
...
function addon.UpdateQuestCompletionData(self)
    ...
    if SkipMissingQuestPreReqs(self, id) then
        element.tooltipText = addon.icons.error .. element.text
        addon.UpdateStepText(self)
        return
    end
...
function addon.functions.hs(self, ...)
    ...
    local event, unit, arg2, arg3 = ...
    local _, spellId, spellName = GetUnitSpellcastData(unit, arg2, arg3)
    local element = self.element
    local step = element.step
    ...
    if event == "CONFIRM_BINDER" then
        self:SetScript("OnUpdate", function()
            ...
            if C_PlayerInteractionManager and
                ...
                Enum.PlayerInteractionType and Enum.PlayerInteractionType.Binder then
                local ok, result = pcall(
                    C_PlayerInteractionManager.ConfirmationInteraction,
                    Enum.PlayerInteractionType.Binder)
                confirmed = ok and result == true
                if confirmed and C_PlayerInteractionManager.ClearInteraction then
                    C_PlayerInteractionManager.ClearInteraction(Enum.PlayerInteractionType.Binder)
                end
            end
            if not confirmed and addon.gameVersion == 30300 and _G.ConfirmBinder then
                _G.ConfirmBinder()
                confirmed = true
            end
            if confirmed then
...
function addon.GetNearestFp()
    ...
    local byZone = addon.FPbyZone and addon.FPbyZone[faction]
    if byZone and mapID and byZone[mapID] then
        return byZone[mapID]
    end

    -- If the taxi window is open, the CURRENT slot is authoritative and avoids
    -- relying on UnitPosition(), which does not exist on 3.3.5a.
    if _G.NumTaxiNodes and addon.TAXIMAP_OPENED then
        addon:TAXIMAP_OPENED()
        for slot = 1, _G.NumTaxiNodes() do
            if _G.TaxiNodeGetType(slot) == "CURRENT" then
                return addon.flightInfo[slot]
            end
        end
    end
end
...
function addon.functions.fp(self, ...)
    if type(self) == "string" then -- on parse
        ...
        if location and location ~= "" and location:find("%w+") then
            if addon.gameVersion == 30300 and addon.ResolveLegacyFlightPath then
                element.fpId, element.fpAmbiguous =
                    addon.ResolveLegacyFlightPath(location)
            else
...
local function NormalizeFlightName(name)
    if type(name) ~= "string" then return nil end
    local normalized = string.lower(name):gsub("[%s%p]+", " ")
...
function addon.functions.fly(self, ...)
    if type(self) == "string" then -- on parse
        ...
        if location and location ~= "" and location:find("%w") then
            ...
            element.locationKey = NormalizeFlightName(location)
            if addon.gameVersion == 30300 and addon.ResolveLegacyFlightPath then
                element.fpId, element.fpAmbiguous =
                    addon.ResolveLegacyFlightPath(location)
            else
                local matchedID
                for id, fp in pairs(addon.FPDB[addon.player.faction] or {}) do
                    local fpName = fp.name and NormalizeFlightName(fp.name)
                    if fpName and element.locationKey and
                        fpName:find(element.locationKey, 1, true) then
                        if matchedID and matchedID ~= id then
                            matchedID = nil
                            element.fpAmbiguous = true
                            break
                        end
                        matchedID = id
                    end
                end
                element.fpId = matchedID
            end
        else
    ...
    elseif automationEnabled and event == "TAXIMAP_OPENED" and element.location then
        ...
        for i = 1, NumTaxiNodes() do
            ...
            local name = _G.TaxiNodeName(i)
            local idMatch = element.fpId and id == element.fpId
            local nodeName = name and NormalizeFlightName(name)
            local nameMatch = not (element.fpId or element.fpAmbiguous) and
                nodeName and element.locationKey and
                nodeName:find(element.locationKey, 1, true)
            if (idMatch or nameMatch) and _G.TaxiNodeGetType(i) == "REACHABLE" then
                matches[#matches + 1] = i
            end
        end
        if #matches == 1 then
            TakeTaxiNode(matches[1])
            if not _G.GameTooltip:IsForbidden() then _G.GameTooltip:Hide() end
            return
        elseif #matches > 1 then
            addon.comms.PrettyDebug("Ambiguous flight destination", element.location)
        else
            addon.comms.PrettyDebug("Flight destination unavailable", element.location)
        end
...
function addon.functions.deathskip(self, ...)
    ...
    if event == "CONFIRM_XP_LOSS" then
        self:SetScript("OnUpdate", function()
            ...
            if C_PlayerInteractionManager and
                C_PlayerInteractionManager.ConfirmationInteraction and Enum and
                Enum.PlayerInteractionType and Enum.PlayerInteractionType.SpiritHealer then
                local ok, result = pcall(
                    C_PlayerInteractionManager.ConfirmationInteraction,
                    Enum.PlayerInteractionType.SpiritHealer)
                confirmed = ok and result == true
                if confirmed and C_PlayerInteractionManager.ClearInteraction then
                    C_PlayerInteractionManager.ClearInteraction(Enum.PlayerInteractionType.SpiritHealer)
                end
            end
            if not confirmed and addon.gameVersion == 30300 and _G.AcceptXPLoss then
                _G.AcceptXPLoss()
                confirmed = true
            end
            if confirmed then
                addon.SetElementComplete(self)
                if _G.StaticPopup1 then _G.StaticPopup1:Hide() end
                if _G.StaticPopup2 then _G.StaticPopup2:Hide() end
            end
            self:SetScript("OnUpdate",nil)
        end)
...
if objFlags is omitted or set to 0, element will complete if you have the quest in your quest log
    ...
    if questId and SkipMissingQuestPreReqs(self, questId) then
        element.tooltipText = addon.icons.error ..
                                  (element.rawtext or element.text or name)
        return
    end
...
function addon.functions.tame(self, ...)
    ...
    if UnitSpellcastMatches(spellId, spellName, 1515) and unit == "player" then
...
function addon.functions.next(skip, guide, arg1)
    ...
    if next then
        ...
        elseif addon.gameVersion == 30300 and skip ~= false then
            ...
            if not legacyCoverageNotified[key] then
                legacyCoverageNotified[key] = true
                addon.comms.PrettyPrint(
                    "End of bundled 3.3.5 guide coverage reached for %s.",
                    addon.GetGuideName(guide) or guide.name or "this route")
            end
        end
    end
end
...
function addon.functions.cast(self, text, ...)
    ...
    local _, id, spellName = GetUnitSpellcastData(unit, arg2, arg3)
    local element = self.element
    ...
    if (event == "UNIT_SPELLCAST_SUCCEEDED" or failed) and unit == element.unit then
        for i,spellId in pairs(element.ids) do
            if UnitSpellcastMatches(id, spellName, spellId) and
                (not failed or element.failcheck[i]) then
...
function addon.functions.holiday(self, text, eventId, reverse)
    ...
    for i = 1, GetNumDayEvents(0, monthDay) do
        ...
        local isDarkmoonEvent = event and
                                    element.eventName ==
                                        _G.CALENDAR_FILTER_DARKMOON and
                                    event.eventID and event.eventID >= 374 and
                                    event.eventID <= 376
        if event and (element.eventId and element.eventId == event.eventID or
            event.title == element.eventName or isDarkmoonEvent) then
            eventFound = true
            break
        end
    end
