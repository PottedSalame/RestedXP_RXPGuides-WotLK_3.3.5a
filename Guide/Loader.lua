...
function addon.LoadEmbeddedGuides()
    ...
    for n, guideData in ipairs(embeddedGuides) do
        if guideData.cache then
            ...
            elseif RXPCData.guideDisabled[n] then
                enabled = false
            end
    ...
    if type(guide) == "table" then
        ...
        --if true then  return end
            ...
            if swap then
                if guide.next then
                    local nextGuides = {}
                    for candidate in guide.next:gmatch("%s*([^;]+)%s*") do
                        local targetGroup = candidate:match("^%s*(.-)\\")
                        if targetGroup and
                            not targetGroup:match("^RestedXP TBC Guide %([AH]%)$") and
                            not targetGroup:match("^RestedXP WotLK Guide %([AH]%)$") then
                            candidate = candidate:gsub("^.-\\", "")
                        end
                        tinsert(nextGuides, candidate)
                    end
                    guide.next = table.concat(nextGuides, ";")
                end
            end
